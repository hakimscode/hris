      ...
      .then((res) => {
        if (res.status === 200) {
          this.setState((prevState) => {
            ...
            detailProject.project_recording_harians = [
              ...this.state.detailProject.project_recording_harians,
              res.data.data,
            ];
              ...
              <CardBody>
                <Row>
                  <Col xs="6" md="6">
                    <ListGroup>
                      <ListGroupItem>
                        {this.state.detailProject.kandang.name} - Periode{" "}
                        {this.state.detailProject.periode}
                      </ListGroupItem>
                      <ListGroupItem>
                        Populasi Awal : {this.state.detailProject.populasi_awal}{" "}
                        ekor
                      </ListGroupItem>
                      <ListGroupItem>
                        Tgl Mulai : {this.state.detailProject.tanggal_mulai}
                      </ListGroupItem>
                      <ListGroupItem>
                        Tgl Closing : {this.state.detailProject.tanggal_closing}
                      </ListGroupItem>
                    </ListGroup>
                  </Col>
                  <Col xs="6" md="6">
                    <ListGroup>
                      ...
                      <ListGroupItem>
                        Total Pakan : {this.state.totalPakan} kg
                      </ListGroupItem>
                      <ListGroupItem>
                        Total Telur : {this.state.totalJumlah} butir
                      </ListGroupItem>
                      <ListGroupItem>
                        Total Tonase Telur : {this.state.totalTonase} kg
                      </ListGroupItem>
                    </ListGroup>
                  </Col>
                </Row>
            ...
            <Card>
              ...
              <CardBody>
                <Table responsive>
                  ...
                  <tbody>
                    {this.state.detailProject.project_recording_harians.map(
                      (row) => (
                        <tr key={row.id}>
                          <td>{row.hari}</td>
                          <td>{row.pakan}</td>
